...
public class CacheUtils {
     ...
     * 设置缓存
     */
    public static synchronized void setCache(String cacheKey, Object cacheValue, long cacheTime) {
        startCleanThread();
        Long ttlTime = null;
        if (cacheTime <= 0L) {
            if (cacheTime == -1L) {
     ...
     * 开启清理过期缓存的线程
     */
    private static synchronized void startCleanThread() {
    ...
    private static class CleanTimeOutThread implements Runnable {
        ...
        public void run() {
            ...
            while (true) {
                LOGGER.debug("check clean cache run ...");
