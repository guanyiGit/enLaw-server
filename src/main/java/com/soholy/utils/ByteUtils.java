...
public class ByteUtils {
    ...
    public static void longTobyte4(long n, byte[] b, int off) {
        if (!(n >= 0 && n <= 4294967295L))
            throw new IllegalArgumentException(n + " exceed unsigned long int range [0,4294967295]!");
        b[off] = 0x00;
        b[off + 1] = 0x00;
        b[off + 2] = 0x00;
        b[off + 3] = 0x00;
     ...
     * @throws Exception
     */
    public static String[] byte2HexStr(byte[] src, String prefix) {
        prefix = prefix == null ? "0x" : prefix;
        String[] sb = new String[src.length];
        for (int i = 0; i < src.length; i++) {
